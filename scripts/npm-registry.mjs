...
async function verify(tarball, distTag) {
  ...
  try {
    ...
    await mkdir(fresh);
    await writeFile(join(fresh, 'package.json'), JSON.stringify({ name: 'rbushweb-public-registry-consumer', version: '1.0.0', private: true, type: 'module' }));
    await writeFile(join(fresh, '.npmrc'), `registry=${registry}\n@wieslawsoltes:registry=${registry}\n`);
    const installed = spawnSync(process.platform === 'win32' ? 'npm.cmd' : 'npm', ['install', `${pkg.name}@${pkg.version}`, '--registry=' + registry, '--cache=' + join(fresh, '.npm-cache'), '--ignore-scripts', '--no-audit', '--no-fund'], { cwd: fresh, stdio: 'inherit', timeout: 180_000, env: { ...process.env, NODE_AUTH_TOKEN: '', NPM_TOKEN: '', NPM_CONFIG_USERCONFIG: join(fresh, '.npmrc') } });
    ...
    const smoke = spawnSync(process.execPath, ['--input-type=module', '-e', `
      import assert from 'node:assert/strict';
      import { RBush, Envelope } from ${JSON.stringify(pkg.name)};
      const items = [
        { id: 'near', Envelope: new Envelope(0, 0, 1, 1) },
        { id: 'middle', Envelope: new Envelope(2, 2, 3, 3) },
        { id: 'far', Envelope: new Envelope(9, 9, 11, 11) },
      ];
      const tree = new RBush();
      tree.BulkLoad(items);
      assert.equal(tree.Count, 3);
      assert.deepEqual(tree.Search(new Envelope(-1, -1, 1, 1)).map(item => item.id), ['near']);
      assert.deepEqual(tree.Knn(1, 10, 10).map(item => item.id), ['far']);
      tree.Delete(items[0]);
      assert.equal(tree.Count, 2);
      assert.equal(tree.Search(new Envelope(-1, -1, 1, 1)).length, 0);
    `], { cwd: fresh, stdio: 'inherit', timeout: 30_000 });
    assert.equal(smoke.status, 0, 'Fresh public npm installation must support bulk loading, search, nearest neighbors, and deletion');
    ...
    await summary(`Published and verified **${pkg.name}@${pkg.version}** on npm (${distTag}).\n\nRelease and registry tarballs have identical SHA512 integrity. Public download, provenance metadata, installed package consumers, and a fresh public npm installation exercising bulk loading, spatial search, nearest neighbors, and deletion passed.\n\n[View npm package](https://www.npmjs.com/package/${pkg.name}/v/${pkg.version})`);
  } finally {
